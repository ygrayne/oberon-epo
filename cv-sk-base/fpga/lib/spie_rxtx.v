/**
  ...
  * slow_div: slow transmit
    ...
    * some makes of SD cards can be very picky about the slow clock frequency...
  
  Odd dividers will result in a non 50%/50% duty-cycle of the serial clock,
  with the low half period being longer by one 'clk' period, which is only
  really relevant for the fast clock frequency.
  --
...
module spie_rxtx #(
  ...
  localparam
    ticks_period_fast = fast_div - 1,
    ticks_period_slow = slow_div - 1,
    ticks_half_period_fast = ticks_period_fast / 2,
    ticks_half_period_slow = ticks_period_slow / 2;
